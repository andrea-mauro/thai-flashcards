        ...
        // Days/Months Quiz State
        let currentDayQuizMode = 'days'; // 'days', 'months', 'both'
        let currentCorrectDayItem = null;
        let dayQuizType = 'visual'; // 'visual' or 'audio'
        ...
        function setupEventListeners() {
            ...
            document.querySelectorAll('.day-range-btn').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    ...
                    e.target.classList.add('active');
                    currentDayQuizMode = e.target.dataset.mode;
                    generateDayQuestion();
                });
            });

            const nextDayBtn = document.getElementById('nextDayBtn');
            if (nextDayBtn) nextDayBtn.addEventListener('click', generateDayQuestion);

            const dayAudioBtn = document.getElementById('dayAudioBtn');
            if (dayAudioBtn) {
                dayAudioBtn.addEventListener('click', () => {
                    if (currentCorrectDayItem) {
                        playAudio(currentCorrectDayItem.thai);
                    }
                });
            }
        ...
        function switchView(viewId) {
            ...
            if (viewId === 'days' && currentCorrectDayItem === null) {
                generateDayQuestion();
            }
        }
        ...
        function toThaiDate(date) {
            ...
            const year = date.getFullYear() + 543; // Thai Solar Calendar

            const dayData = toThai(dayOfMonth);
            const yearData = toThai(year);
        ...
        function generateDayQuestion() {
            let pool = [];
            if (currentDayQuizMode === 'days') pool = thaiDays;
            else if (currentDayQuizMode === 'months') pool = thaiMonths;
            else pool = [...thaiDays, ...thaiMonths];

            currentCorrectDayItem = pool[Math.floor(Math.random() * pool.length)];
            dayQuizType = Math.random() > 0.5 ? 'visual' : 'audio';
            ...
            if (dayQuizType === 'visual') {
                questionEl.textContent = currentCorrectDayItem.english;
                typeLabel.textContent = 'What is this in Thai?';
                audioVis.style.display = 'none';
                questionEl.style.fontSize = '3rem';
            } else {
                questionEl.innerHTML = `<div>${currentCorrectDayItem.thai}</div><div style="font-size: 1.2rem; color: #7f8c8d; font-weight: normal; margin-top: 10px;">${currentCorrectDayItem.roman}</div>`;
            ...
            // Generate choices
            let choices = [currentCorrectDayItem];
            ...
            choices.forEach(item => {
                ...
                btn.onclick = () => checkDayAnswer(item, btn);
                choicesEl.appendChild(btn);
            });
        }

        function checkDayAnswer(selected, btn) {
            if (document.querySelector('#dayChoices .choice-btn.correct')) return;

            const feedback = document.getElementById('quizFeedback'); // Wait, ID is dayFeedback
            const realFeedback = document.getElementById('dayFeedback');
            const nextBtn = document.getElementById('nextDayBtn');
            if (!realFeedback || !nextBtn) return;

            if (selected.id === currentCorrectDayItem.id && typeof selected.english === typeof currentCorrectDayItem.english) {
                btn.classList.add('correct');
                realFeedback.innerHTML = `Correct! <strong>${currentCorrectDayItem.thai}</strong> (${currentCorrectDayItem.roman})`;
                realFeedback.className = 'quiz-feedback correct';
                playAudio(currentCorrectDayItem.thai);
            } else {
                btn.classList.add('incorrect');
                realFeedback.innerHTML = `Incorrect. It was <strong>${currentCorrectDayItem.thai}</strong>`;
                realFeedback.className = 'quiz-feedback incorrect';
                
                document.querySelectorAll('#dayChoices .choice-btn').forEach(b => {
                    if (dayQuizType === 'visual') {
                        if (b.innerText.includes(currentCorrectDayItem.thai)) b.classList.add('correct');
                    } else {
                        if (b.textContent === currentCorrectDayItem.english) b.classList.add('correct');
                    }
                });
            }
